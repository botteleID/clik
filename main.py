...
async def cbconfess(client, query: CallbackQuery):
  ...
  confesss = isi
    report = await client.send_message(DEV, f"from {query.from_user.mention}\nisi : {confesss.text}", disable_web_page_preview=True)
await client.send_message(LOG, f"<b>From :</b> <i>{nama}</i>\n<b>To :</b> <i>{to.text}</i>\n<b>Isi :</b> <i>{confesss.text}</i>", disable_web_page_preview=True)
